...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  } //terminating condition is on Empty.filterAcc()... Which simply returns acc when it is reached
  ...
  override def mostRetweeted: Tweet = {
    var tmpMax = elem
    if (!left.empty() && left.mostRetweeted.retweets > tmpMax.retweets) {
      tmpMax = left.mostRetweeted
    }

    if (!right.empty() && right.mostRetweeted.retweets > tmpMax.retweets) {
      tmpMax = right.mostRetweeted
    }
